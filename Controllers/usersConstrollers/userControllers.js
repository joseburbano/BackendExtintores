...
exports.signUp = (req, res) => {
  ...
  if (!password || !repeatPassword) {
    res
      .status(404)
      .json({ code: 404, message: "Las contraseñas son obligatorias" });
  } else {
    if (password !== repeatPassword) {
      res
        .status(404)
        .json({ code: 404, message: "Las contraseñas no son iguales" });
    } else {
      bcrypt.hash(password, null, null, function (err, hast) {
        if (err) {
          res
            .status(500)
            .json({ code: 500, message: "Error al encriptar la contraseña" });
        } else {
          user.password = hast;
          emails = user.email;
          ...
          user.save((err, userStored) => {
            if (err) {
              res
                .status(500)
                .json({ code: 500, message: "El usuario ya existe." });
            } else {
              if (!userStored) {
                res
                  .status(404)
                  .json({ code: 404, message: "Error al crear Usuario." });
              } else {
                res.status(200).json({
                  code: 200,
