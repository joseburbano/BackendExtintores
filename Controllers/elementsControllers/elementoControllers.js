const Elementos = require("../../models/Elemento");
...
exports.addExtintor = async (req, res) => {
  try {
    ...
    const extintors = new Elementos(body);
    ...
    await extintors.save((err, extintorStored) => {
      if (err) {
        ...
        if (!extintorStored) {
          ...
          user.elementos.push(extintors._id);
...
exports.uploadFoto = (req, res) => {
  ...
  Elementos.findById({ _id: params.id }, (err, userData) => {
    if (err) {
...
exports.getExtintores = (req, res) => {
  ...
  Elementos.paginate({}, options, (err, postsStored) => {
    if (err) {
...
exports.updateExtintor = (req, res) => {
  ...
  const { id } = req.params;
  Elementos.findByIdAndUpdate(id, extintorData, (err, extintorUpdate) => {
    if (err) {
      res.status(500).json({ code: 500, message: "Error del servidor." });
    } else {
      if (!extintorUpdate) {
...
exports.deleteExtintor = (req, res) => {
  const { id } = req.params;
  Elementos.findOne({ _id: id }, (err, postStored) => {
    if (err) {
      res.status(500).send({ code: 500, message: "Error del servidor." });
    } else {
      if (!postStored) {
          ...
          (err, extintorUpdate) => {
            if (err) {
              ...
              if (!extintorUpdate) {
                ...
                Elementos.findByIdAndRemove(id, (err, extintorDeleted) => {
                  if (err) {
...
exports.getExtintor = (req, res) => {
  ...
  Elementos.findOne({ url }, (err, extintorStored) => {
    if (err) {
      res.status(500).send({ code: 500, message: "Error del servidor." });
    } else {
      if (!extintorStored) {
...
exports.getExtintorId = (req, res) => {
  ...
  Elementos.findById(id, (err, extintorStored) => {
    if (err) {
      res.status(500).send({ code: 500, message: "Error del servidor." });
    } else {
      if (!extintorStored) {
...
exports.consulSede = (req, res) => {
  ...
  Elementos.paginate({ sede: sede }, options, (err, extintores) => {
    if (err) {
      res.status(500).send({ code: 500, message: "Error del servidor." });
    } else {
      if (!extintores) {
